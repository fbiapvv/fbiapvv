...
def parse_events(sock, loop_count=1000):
    ...
    for i in range(0, loop_count):
        ...
        if dataString[38:46] == '4c000215':
            ...
            resultsArray = [{ "datetime": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "id" : macAddress[-5:].replace(':', ''), "type": type, "uuid": uuid, "major": majorVal, "minor": minorVal, "rssi": rssi, "macAddress": macAddress}]
        ...
        resultsArray = [{ "datetime": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "id" : macAddress[-5:].replace(':', ''), "type": type, "rssi": rssi, "macAddress": macAddress}]
